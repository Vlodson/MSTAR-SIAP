...
def __parse_header(phoenix_header: Dict[str, str]) -> Header:
    return {
        ...
        "angle": float(phoenix_header["DesiredDepression"]),
        "azimuth": float(phoenix_header["TargetAz"]),
...
def read_sar_image(path: str) -> Optional[Image]:
...
def save_sar_image_as_pkl(path: str, image: Image) -> None:
